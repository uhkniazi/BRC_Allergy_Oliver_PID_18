...
########################################################################################
##### peanut specific activity, isac shannon diversity and avidity
########################################################################################
## see pages 105 to 109 (rethinking book) for extract.samples, link and sim for 
## posterior predictive checks 
str(dfData)
dfData$CD63_Act = dfData$CD63.Act - mean(dfData$CD63.Act)
dfData$Peanut_Sp_Act = dfData$Peanut.Sp.Act - mean(dfData$Peanut.Sp.Act)
dfData$ISAC_Shannon = dfData$ISAC.Shannon - mean(dfData$ISAC.Shannon)

fit.lm = lm(CD63_Act ~ Peanut_Sp_Act + ISAC_Shannon + Avidity, data=dfData)
summary(fit.lm)

model.1 <- alist(
  # https://github.com/rmcelreath/rethinking/blob/ae3fae963e244c58bbd68dd86478840a87d29f49/man/dstudent.Rd#L37
  CD63_Act ~ dstudent(3, mu, sigmaPop),
  mu <- b0 + b_Peanut_Sp_Act * Peanut_Sp_Act +
    b_ISAC_Shannon * ISAC_Shannon +
    b_Avidity * Avidity,
  b0 ~ dnorm(0, 2),
  c(b_Peanut_Sp_Act, b_ISAC_Shannon, b_Avidity) ~ dnorm(0, 3),
  sigmaPop ~ dexp(1/28)
  # see here for example
  # https://github.com/rmcelreath/rethinking/blob/ae3fae963e244c58bbd68dd86478840a87d29f49/tests/rethinking_tests/test_ulam1.R#L20
  #nu ~ gamma(3, 0.1)
...
summary(fit.1)
plot(coeftab(fit.1), pars=c('b0', 'b_Peanut_Sp_Act', 'b_ISAC_Shannon', 'b_Avidity'))
...
## checking for non-linearity
plot(dfData$Peanut_Sp_Act, iResid.pa)
lines(lowess(dfData$Peanut_Sp_Act, iResid.pa))

plot(dfData$ISAC_Shannon, iResid.pa)
lines(lowess(dfData$ISAC_Shannon, iResid.pa))

plot(dfData$Avidity, iResid.pa)
lines(lowess(dfData$Avidity, iResid.pa))

## unequal variances
plot(dfData$Peanut_Sp_Act, abs(iResid.pa))
lines(lowess(dfData$Peanut_Sp_Act, abs(iResid.pa)))

plot(dfData$ISAC_Shannon, abs(iResid.pa))
lines(lowess(dfData$ISAC_Shannon, abs(iResid.pa)))
...
i = range(dfData$Peanut_Sp_Act)
iGrid = seq(i[1], i[2], length.out = 50)
## hold 2 variables at average
coef(fit.1)
mFitted = link(fit.1, data=list(Peanut_Sp_Act=iGrid, 
                                ISAC_Shannon = rep(0, times=50),
                                Avidity=rep(0, times=50)))
## posterior predictive values for fitted
lines(iGrid, colMeans(mFitted), col='green')
mu.hpdi = apply(mFitted, 2, HPDI)
...
i = range(dfData$ISAC_Shannon)
iGrid = seq(i[1], i[2], length.out = 50)
## hold other variables at average
coef(fit.1)
mFitted = link(fit.1, data=list(Peanut_Sp_Act=rep(0, times=50), 
                                ISAC_Shannon = iGrid,
                                Avidity=rep(0, times=50)))
## posterior predictive values for fitted
lines(iGrid, colMeans(mFitted), col='green')
